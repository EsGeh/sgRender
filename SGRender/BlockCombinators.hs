...
import SGRender.TestConditions
...
renderListHoriWithSep fillFSep sepWidth divBlocks listRenderMeth =
	...
	where
		renderInterspersed' = renderListHori divBlocks $ renderInterspersed renderSep listRenderMeth
			where
				renderSep = renderToConstRepr
					fillFSep 	-- :: Size Int -> Block Char
					dimRelSep	-- :: DimRel Int
...
renderListVertWithSep fillFSep sepWidth divBlocks listRenderMeth = renderMeth (\size src -> renderF renderInterspersed' size (interspersedSrc src)) (\src -> srcInfo renderInterspersed' (interspersedSrc src))
	where
		renderInterspersed' = renderListVert divBlocks $ renderInterspersed renderSep listRenderMeth
			where
...
{-
divWithSep :: Int -> DivBlocks Int-> DivBlocks Int
--divWithSep :: Int -> ((t, Int) -> [a] -> [Int]) -> (t, Int) -> [a] -> [Int]
